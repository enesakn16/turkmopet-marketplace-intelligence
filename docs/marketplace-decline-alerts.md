...
Otomatik yaşam döngüsü geçişleri `marketplace_decline_task_events` tablosuna eklemeli denetim kaydı olarak yazılır. Yeni kritik görev ilk kez oluşturulduğunda `CREATED_CRITICAL` olayı `ABSENT → OPEN` geçişini kaydeder. Sonraki `AUTO_RESOLVED` ve `REOPENED_CRITICAL` olayları da aynı görev anahtarının geçmişine eklenir. Aynı kritik rapor tekrar işlendiğinde ikinci bir oluşturma olayı yazılmaz. Her kayıt görev anahtarını, olay türünü, önceki/yeni durumu, açıklamayı ve zaman damgasını taşır. Manuel `RESOLVED` kararları sistem tarafından değiştirilmez ve bunlar için otomatik kapanma/yeniden açılma olayı üretilmez.
